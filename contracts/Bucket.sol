  ...
  ParticipantInteractor
{
  ...
  // Contains all hashes ever saved to this bucket
  Element[] public allElements;
  // old version points to new version (hashes) -> possibility to go forwards in history
  mapping(string => string) public oldToNewVersions;
  // new version points to old version (hashes) -> possibility to go backwards in history
  mapping(string => string) public newToOldVersions;
  // child points to parent (hashes) -> possibility to reproduce hierarchy
  mapping(string => string) public parent;
  // parent points to children (hashes) -> possibility to reproduce hierarchy
  mapping(string => string[]) public children;
  ...
  function _createElement(
    ...
    _addElement(newContainerHash, contentType, ElementType.CONTAINER, index);
  ...
  function _updateElement(
    ...
    if (
      keccak256(bytes(newContainerHash)) != keccak256(bytes(prevContainerHash))
    ) {
      _addElement(newContainerHash, contentType, ElementType.CONTAINER, index);
  ...
  function _removeElement(
    ...
    require(
      keccak256(bytes(oldToNewVersions[metaHash])) == keccak256(bytes('')),
      'Newer (meta) version exists!'
    );
    require(
      keccak256(bytes(oldToNewVersions[dataHash])) == keccak256(bytes('')),
      'Newer (data) version exists!'
    );
    require(
      keccak256(bytes(oldToNewVersions[containerHash])) == keccak256(bytes('')),
  ...
  function _checkVersioning(string memory oldVersion, string memory newVersion)
    private
    view
  {
    require(
      keccak256(bytes(oldToNewVersions[oldVersion])) == keccak256(bytes('')),
      'Old version already has an update!'
    );
    require(
      keccak256(bytes(newToOldVersions[newVersion])) == keccak256(bytes('')),
  ...
  function _checkParent(
    ...
    require(
      ...
      string.concat('Element (', name, ') is not child of ', parentHash, '!')
    );
  }
  ...
  function _updateVersioning(string memory oldVersion, string memory newVersion)
    private
  {
