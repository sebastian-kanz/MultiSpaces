...
contract PaymentManager is IPaymentManager, Ownable {
    ...
    mapping(address => bool) public limitsInitialized;
    ...
    constructor(uint256 baseFee, uint256 baseLimit, uint256 price) {
    ...
    {
        PaymentState memory state;
        state.limit = getLimit(account, LimitedAction.ADD_DATA);
        state.unlimited = isUnlimited(account, LimitedAction.ADD_DATA);
        state.balance = getBalance(account);
        state.createSpaceVoucherCount = getVoucherCount(
            account,
            PayableAction.CREATE_SPACE
        );
        state.addBucketVoucherCount = getVoucherCount(
            account,
            PayableAction.ADD_BUCKET
        );
        state.addParticipantVoucherCount = getVoucherCount(
            account,
            PayableAction.ADD_PARTICIPANT
        );
        state.createSpaceIsFreeOfCharge = isFreeOfCharge(
            account,
            PayableAction.CREATE_SPACE
        );
        state.addBucketIsFreeOfCharge = isFreeOfCharge(
            account,
            PayableAction.ADD_BUCKET
        );
        state.addParticipantIsFreeOfCharge = isFreeOfCharge(
            account,
            PayableAction.ADD_PARTICIPANT
        );
        state.defaultLimit = DEFAULT_LIMITS[LimitedAction.ADD_DATA];
        state.defaultPayment = DEFAULT_PAYMENTS[PayableAction.ADD_BUCKET];
        return (
            state.limit,
            state.unlimited,
            state.balance,
            state.createSpaceVoucherCount,
            state.addBucketVoucherCount,
            state.addParticipantVoucherCount,
            state.createSpaceIsFreeOfCharge,
            state.addBucketIsFreeOfCharge,
            state.addParticipantIsFreeOfCharge,
            state.defaultLimit,
